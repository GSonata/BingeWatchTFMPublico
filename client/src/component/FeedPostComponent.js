...
import { faHeart, faTrash, faComment, faFilm, faBox } from '@fortawesome/free-solid-svg-icons';
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';

import "../styles/FeedPostComponent.scss"
                    ...
                    <div className="activity-card" data-type="badge" data-username={item.alias}>
                        ...
                        <div className="activity-content">
                            ...
                            <button
                                ...
                                onClick={handleLike}
                            >
                                <FontAwesomeIcon icon={faHeart} /> {item.likes?.length || 0}
                            </button>
                        </div>
                    </div>
                ) : (
                            ...
                            <Link to={`/movies/${item.imdbID}`} className="activity-link">
                                ...
                                {(item.soporte || item.estado) && (
                                    <p className="activity-extra">
                                        {item.soporte && <span><FontAwesomeIcon icon={faFilm} /> {item.soporte}</span>}
                                        {item.estado && <span> • <FontAwesomeIcon icon={faBox} /> {item.estado}</span>}
                                    </p>
                                )}
                            </Link>
